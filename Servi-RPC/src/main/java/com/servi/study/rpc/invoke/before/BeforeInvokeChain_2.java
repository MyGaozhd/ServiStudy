...
public class BeforeInvokeChain_2 extends AbstractInvokeChain implements IBeforeInvoke {
    ...
    public void excute(IRequest req, IResponse res) {
        System.out.println(BeforeInvokeChain_2.class.getName() + "->" + order() + "->" + req.getClass().toString());
        System.out.println(BeforeInvokeChain_2.class.getName() + "->" + order() + "->" + res.getClass().toString());
        if (next() != null) {
            next().excute(req, res);
        }
    }
